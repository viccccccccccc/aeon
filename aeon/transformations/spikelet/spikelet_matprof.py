...
def Spikelet_MP_new_ver_02(MagInfo, MP_param):
    ...
    if 'prune_method' in MP_param:
        if MP_param['prune_method'] == 'first_spike':
            for i in range(len(StringMatrix_cell)):
                ...
                MagInfo, SpikeDb = Spikelet_SpikeDb_statistics(MagInfo, StatVarArray, SpikeSet)
                for j, pq in enumerate(PQ_array):
                    ...
                    if type_ == "percentile":
                        ...
                        feature_string_index = np.argsort(MagInfo['mp']['index'][feature])
                        MagInfo['mp']['index'][f"{feature}_sorted_val"] = feature_string_val
                        MagInfo['mp']['index'][f"{feature}_sorted_index"] = feature_string_index
    ...
    for i, StringDb in enumerate(StringMatrix_cell):
        ...
        string_matrix_i, string_matrix_names, MP_trend_raw_i, MP_trend_norm_i, dist_calc_num, TOPK_norm, TOPK_names = calculateMP(
            StringDb, StringMatrix_names, MP_param, MagInfo
        )
        
        if string_matrix_i is not None:
            Op2 = 'calculate_MP'
            MagInfo['size'][OpDetail][Op2][i, :] = [dist_calc_num['before'], dist_calc_num['after']]
            MagInfo['size'][Op][i, :] = [dist_calc_num['before'], dist_calc_num['after']]
            MagInfo['time'][OpDetail][Op2][i] = time.time() - start_SMCi
            
            if 'prune_method' in MP_param:
                MagInfo['time'][Op][i] = MagInfo['time'][OpDetail][Op1][i] + MagInfo['time'][OpDetail][Op2][i]
            else:
                MagInfo['time'][Op][i] = MagInfo['time'][OpDetail][Op2][i]
            
            print(f'MP_[{i}:{QR["query"][i]["pattern"]}] the number of distance calculation {dist_calc_num["before"]} -> {dist_calc_num["after"]}')
            print(f'MP_[{i}:{QR["query"][i]["pattern"]}] time = {MagInfo["time"][Op][i]}')
        else:
            print(f'no words\n')
            print(f'MP_[{i}:{QR["query"][i]["pattern"]}] time = {MagInfo["time"][Op][i]}')
...
def calculateMP(StringMatrix, StringMatrix_names, MP_param, MagInfo):
    TOPK_names = ["score", "string_id_1", "string_id_2", "from_1", "to_1", "from_2", "to_2"]
    ...
    for i in range(len(StringMatrix)):
        ...
        StringId_index_i = StringDb_prune(i, StringMatrix, StringMatrix_names, MP_param, MagInfo)
        ...
        if len(StringId_list_i) == 0:
            ...
            for string_id_ij in StringId_list_i:
                ...
                if ((from_j <= from_i and from_i < to_j) or (from_i <= from_j and from_j < to_i)):
                    dist_raw_ij = np.inf
                    dist_norm_ij = np.inf
                else:
                    dist_raw_ij, dist_norm_ij = pair_distance(Si, Sj, MP_param['distance'])
                ...
                if 'TOPK_norm' in locals():
                    if TOPK_thr == np.inf and dist_norm_ij != np.inf:
                        TOPK_pos += 1
                        TOPK_norm[TOPK_pos - 1] = [dist_norm_ij, i, string_id_ij, from_i, to_i, from_j, to_j]
                        if TOPK_pos == TopK:
            ...
            MP_trend_norm[int(from_i):int(to_i) + 1] = np.minimum(MP_trend_norm[int(from_i):int(to_i) + 1],
                                                                  MP[i, MP_names.index("nn_dist_norm")])
            MP_trend_raw[int(from_i):int(to_i) + 1] = np.minimum(MP_trend_raw[int(from_i):int(to_i) + 1],
    ...
    return MP, MP_names, MP_trend_raw, MP_trend_norm, distcalc_num, TOPK_norm, TOPK_names
    ...
    if Prune_param['prune_method'] == 'first_spike':
        ...
        for query_i in Prune_param['prune_query']:
            ...
            if type_ == "range":
                ...
                pos_sorted = np.where(MagInfo['mp']['index'][f"{var}_sorted_index"] == string_id)[0][0]
                lower_pos_sorted = int(np.ceil(spike_id_all_length * (percentile / 100) * (pos_sorted) / spike_id_all_length))
                upper_pos_sorted = int(np.ceil(spike_id_all_length * (percentile / 100) * (spike_id_all_length - pos_sorted) / spike_id_all_length))
                from_sorted = max(0, pos_sorted - lower_pos_sorted)
                to_sorted = min(spike_id_all_length, pos_sorted + upper_pos_sorted)
                Selected_index = MagInfo['mp']['index'][f"{var}_sorted_index"][from_sorted:to_sorted]
