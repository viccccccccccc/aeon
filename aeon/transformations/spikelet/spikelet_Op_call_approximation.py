...
def Spikelet_Op_reduceSpikeByMagnitude(MagInfo):
    ...
    if Method == 'manual':
        ...
        if METHOD_auto == 'initial':
            ...
            #for i in range(100):
            #    print(f"Mag[{i}] = {MagInfo_tmp['magnitude'][i]}")
...
def Spikelet_restrictSupportByWindowLength(MagInfo):
    ...
    M_formula = ParamSRB.get('query', 'Mag != 0')  # Default to a condition that is always true
...
def support_restriction_by_magnitude_ratio(Data_org, Mag_org, Left_org, Right_org, ParamSRM):
    ...
    for t in M_pos:
        ...
        left_mag_rel = np.where(
            (Mag_org_rel * np.sign(Mag_org[t]) > 0) & (np.abs(Mag_org_rel) > np.abs(Mag_org[t]) * MRatio)
        )[0]
