...
def Spikelet_Word_query(MagInfo, Query):
    ...
    for i, Query_i in enumerate(Query):
        ...
        method_left = (
            Query_i["terminal_left"].split("_")
            if "_" in Query_i["terminal_left"]
            else [Query_i["terminal_left"]]
        )
        LRML_left = f"left_{method_left[0]}"
        percentile_left = float(method_left[1]) if len(method_left) > 1 else np.nan

        method_right = (
            Query_i["terminal_right"].split("_")
            if "_" in Query_i["terminal_right"]
            else [Query_i["terminal_right"]]
        )
        ...
        if DEBUG_Word_support_length:
            ...
            plot_Word_support_length(
                startIndex_i, endIndex_i, CMS_str, Query_i["pattern"], SpikeDb, Data_org
            )
        ...
        if Query_i.get("constraint"):
            ...
            for j, (start, end) in enumerate(Scand_i):
                for spike_id_k in range(start, end + 1):
                    ...
                    constraint_ijk = get_constraint(
                        order_ijk, symbol_i, Query_i["pattern"], Query_i["constraint"]
                    )
        ...
        print(
            f"{Op}_{i + 1}: size = {MagInfo['size'][Op][i, 0]} -> {MagInfo['size'][Op][i, 1]}"
        )
        ...
        string_i[:, string_query_names.index("local_id")] = np.arange(
            1, len(startIndex_i) + 1
        )
        ...
        string_i[:, string_query_names.index("char_length")] = (
            endIndex_i - startIndex_i + 1
        )
        string_i[:, string_query_names.index("from_org")] = SpikeDb["from"][
            startIndex_i
        ]
        string_i[:, string_query_names.index("to_org")] = SpikeDb["to"][endIndex_i]
        string_i[:, string_query_names.index("from_percentile")] = extract_terminal(
            SpikeDb["center"][startIndex_i], LRML_left, percentile_left, MagInfo
        )
        string_i[:, string_query_names.index("to_percentile")] = extract_terminal(
            SpikeDb["center"][endIndex_i], LRML_right, percentile_right, MagInfo
        )
        string_i[:, string_query_names.index("from")] = extract_terminal(
            SpikeDb["center"][startIndex_i], LRML_left, percentile_left, MagInfo
        )
        string_i[:, string_query_names.index("to")] = extract_terminal(
            SpikeDb["center"][endIndex_i], LRML_right, percentile_right, MagInfo
        )
        string_i[:, string_query_names.index("word_length")] = (
            string_i[:, string_query_names.index("to")]
            - string_i[:, string_query_names.index("from")]
            + 1
        )

        # Debugging plot
        if DEBUG_Word_support_length:
            plot_Word_support_length_time(
                string_i[:, string_query_names.index("from_percentile")],
                string_i[:, string_query_names.index("to_percentile")],
                Data_org,
            )
        ...
        print(
            string_i[
                ~Index_SuppCheck_i,
                [string_query_names.index("from"), string_query_names.index("to")],
            ]
        )
        ...
        for j in range(len(string_i)):
            ...
            string_info_ij = {
                ...
                "time_index": (
                    SpikeDb["from"][startIndex_i[j]],
                    SpikeDb["to"][endIndex_i[j]],
                ),
                "center_list": SpikeDb["center"][startIndex_i[j] : endIndex_i[j] + 1],
                "type_list": SpikeDb["type"][startIndex_i[j] : endIndex_i[j] + 1],
                "terminal_index": (
                    string_i[j, string_query_names.index("from")],
                    string_i[j, string_query_names.index("to")],
                ),
            }
            ...
            string_feature_i[j, :] = Spikelet_String_features(
                zscore(Data_org), Peaks_ij, string_feature_names
            )
    ...
    if Debug:
        for info in string_info_i:
            print(
                f"[{info['time_index'][0]} - {info['time_index'][1]}] {info['alphabet']}"
            )
        ...
        for spike_id in Index:
            print(
                f"[{SpikeDb['from'][spike_id]} {SpikeDb['to'][spike_id]}] {SpikeDb['type'][spike_id]}"
            )
...
def peaks_in_string(string_info):
    ...
    for i, type_val in enumerate(type_list):
        if type_val != 0 or (
            type_val == 0
            and i >= 1
            and i < len(type_list) - 1
            and type_list[i - 1] * type_list[i + 1] == 1
        ):
...
def plot_Word_support_length(
    startIndex, endIndex, CMS_str, patternDef, SpikeDb, Data_org
):
    LINEWIDTH = 2
    ...
    for i in range(len(from_id_list)):
        ...
        MFT_word = np.column_stack(
            (
                SpikeDb["magnitude"][startIndex[i] : endIndex[i] + 1],
