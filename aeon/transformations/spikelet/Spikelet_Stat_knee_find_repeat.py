...
def Spikelet_Stat_knee_find_repeat(MagDist, Func):
    ...
    if isinstance(Func, (list, str)):
        if isinstance(Func, str) or len(Func) == 1:
            ...
            print(f"[{Spikelet_Stat_knee_find_repeat.__name__}] unknown approx_function length ({len(Func)})")
            return None, None
    else:
        print(f"[{Spikelet_Stat_knee_find_repeat.__name__}] unknown approx_function datatype")
        return None, None
