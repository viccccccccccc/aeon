...
def Spikelet_MpParam_generate_ver_02(user):
    ...
    op_nc = ['reduceSpikeByMagnitude', 'restrictSupportByWindowLength', 'restrictSupportByMagnitudeRatio']
    ...
    # Plot parameters
    param['plot_param'] = Spikelet_SD_createTrendPlotParam(param['operation']['operation_sequence'])

    # Magnitude threshold
    if hasattr(user, 'magnitude_threshold_auto'):
        param['operation']['reduceSpikeByMagnitude']['method'] = user.magnitude_threshold_auto
    ...
    if hasattr(user, 'constant_length_threshold'):
        ...
        param['operation']['extractConstantSegment']['lengthThreshold'] = {'method': 'auto'}

    # Restrict support by window length
    if hasattr(user, 'supp_max'):
        param['operation']['reduceSpikeByMagnitude']['auto']['op']['restrictSupportByWindowLength']['window_band'] = [user.supp_max, user.supp_max]
...
def default_param(supp_min, supp_max):
    ...
    op = {
        'operation_sequence': ['reduceSpikeByMagnitude', 'restrictSupportByMagnitudeRatio', 'extractConstantSegment', 'reduceSpikeByMagnitudeRatio'],
        'name': 'standard_C',
        'reduceSpikeByMagnitude': {
            'auto': {
                ...
                'op': {
                    ...
                    'restrictSupportByWindowLength': {'window_band': [supp_max, supp_max]}
                }
            }
        },
    ...
    spike_db_param = {
        'symbol_mapping': {'rule': symbol_mapping_rule, 'supp_max': supp_max, 'supp_min': supp_min},
        'index': [],
        'reduction_query': ''
    }
...
def Spikelet_SD_createTrendPlotParam(op_seq):
    ...
    tt = ['org_tm', f'{op1}_tm'] + [f'{op}_tm' for op in op_seq] + ['_symbol', 'org_tmMotif']
