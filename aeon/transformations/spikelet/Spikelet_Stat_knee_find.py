...
from aeon.transformations.spikelet.Spikelet_Stat_data2distribution import Spikelet_Stat_data2distribution
...
def Spikelet_Stat_knee_find(MagDist, Func, Weight=None):
    ...
    print(f"Func_fwd: {Func_fwd}, Dim_fwd: {Dim_fwd}, Func_bwd: {Func_bwd}, Dim_bwd: {Dim_bwd}")
    print(f"MagDist: {len(MagDist)}, D2D_arg_weight: {D2D_arg_weight}, StepDivLength: {StepDivLength}")
...
def Spikelet_Stat_knee_find_double(MagDist, FuncList, Weight=None):
    ...
    KeeOptover_pos = np.where(Info_2['Y_fwd'] >= np.max(Info_2['Y_bwd']))[0][-1] if len(np.where(Info_2['Y_fwd'] >= np.max(Info_2['Y_bwd']))[0]) > 0 else len(Info_2['Y_fwd'])
