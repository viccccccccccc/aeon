...
def Spikelet_Stat_knee_find_of_mag_using_support_knee(
    M, S, Support_knee_func, Func, Weight=None
):
    ...
    if isinstance(Support_knee_func, (list, str)):
        if len(Support_knee_func) == 1:
            KneeOpt_S, Info_S = Spikelet_Stat_knee_find(
                S, Support_knee_func
            )  # no weight
        elif len(Support_knee_func) == 2:
            KneeOpt_S, Info_S = Spikelet_Stat_knee_find_double(
                S, Support_knee_func
            )  # no weight
        else:
            print(
                f"[{FuncName}] unknown support_knee_func length ({len(Support_knee_func)})"
            )
            return None, None
