...
from aeon.transformations.spikelet.Spikelet_Stat_knee_find_of_mag_using_magband_support import Spikelet_Stat_knee_find_of_mag_using_magband_support
from aeon.transformations.spikelet.Spikelet_Stat_knee_find_of_mag_using_support_knee import Spikelet_Stat_knee_find_of_mag_using_support_knee
...
def Spikelet_Stat_magnitude_threshold_SuppMax_SuppMRatio(MagInfo, ParamMagThr):
    ...
    if Type == 'simple':
        ...
        MagThr, KneeInfo = Spikelet_Stat_knee_find_of_mag_using_magband_support(Mag_selected, Supp_selected, Func, BandFunc)
    elif Type == 'support_knee':
        Support_knee_func = ParamMagThr['knee_find'].get('support_knee_func', 'poly_1_0')
        
        if not ParamMagThr['knee_find'].get('weight') or (ParamMagThr['knee_find'].get('weight') == 'none'):
            Weight = np.nan
        elif ParamMagThr['knee_find'].get('weight') == 'support_length':
            Weight = Supp_selected
        
        MagThr, KneeInfo = Spikelet_Stat_knee_find_of_mag_using_support_knee(Mag_selected, Supp_selected, Support_knee_func, Func, Weight)
    else:
