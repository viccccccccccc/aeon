...
from aeon.transformations.spikelet.spikelet_generateInitialSpikelet import Spikelet_generateInitialSpikelet
...
def Spikelet_aproximation_ver_03(X, Param):
    ...
    for i in range(100):
        print(f"Mag[{i}] = {MagInfo['magnitude'][i]}")
    ...
    MagInfo['size'][InitialOpName] = np.sum(~np.isnan(MagInfo['output'][InitialOpName]['type']))
    MagInfo['param'] = Param
